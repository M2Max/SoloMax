...
export default async function BlogIndex({
  ...
  if (!articles || articles.length === 0) {
    return (
      <div className="flex flex-col items-center bg-background justify-center min-h-[calc(100vh-200px)] py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl w-full space-y-8 bg-background text-center">
  ...
  return (
    <div className="flex flex-col items-center min-h-[calc(100vh-200px)] py-12 px-4 sm:px-6 lg:px-8 bg-background dark:bg-my-black">
      <div className="max-w-3xl w-full space-y-8">
        ...
        <ul className="space-y-8">
          {articles.map((article) => {
              ...
              return (
                <li key={article.slug} className="bg-background dark:bg-my-black shadow-lg rounded-lg overflow-hidden flex flex-col sm:flex-row items-start sm:items-center hover:shadow-xl transition-shadow duration-300 ease-in-out ring-1 ring-gray-100 dark:ring-gray-700">
                  {article.image && (
                  ...
                  <div className="flex-grow p-5">
                    <Link href={articleLink} className="group">
                      <h2 className="text-2xl font-semibold text-my-black dark:text-my-black group-hover:text-primary dark:group-hover:text-primary transition-colors duration-200 mb-2">
