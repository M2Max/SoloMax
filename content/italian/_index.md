---
# Banner
banner:
  title: "Ciao, sono Max"
  content: "AI Engineer e appassionato di tecnologia"
  image: "/images/banner.png"

# Features
features:
  - title: "Benvenuto nel mio spazio digitale!"
    image: "/images/CodeWithAI.png"
    content: >-
      Mi occupo di _**sviluppo software e intelligenza artificiale**_. In questo sito raccolgo quello che imparo ogni giorno: progetti che sto seguendo, soluzioni a problemi tecnici che ho incontrato e riflessioni su come sta cambiando la tecnologia. <br><br>

      _**Non**_ mi interessa la teoria fine a se stessa: mi piace sporcarmi le mani, testare cosa funziona davvero e scartare quello che è solo marketing. _**Se cerchi un punto di vista pratico**_ su come usare queste tecnologie, _**sei nel posto giusto**_.

  - title: "Come posso aiutarti?"
    image: "/images/AIAutomation.png"
    content: >-
      Posso _**supportarti nell’uso dell’intelligenza artificiale in modo pratico e mirato**_. <br>
      Ti aiuto a _**automatizzare processi**_, a valutare se l’AI è davvero utile per il tuo caso e a _**sviluppare progetti completi che integrano l’AI**_, dall’idea iniziale alla messa in produzione.<br><br>

      Lavoro anche su _**modelli addestrati sui tuoi dati**_, con possibilità di _**deploy in locale**_ su infrastrutture dedicate, così da garantire controllo totale e _**massima privacy**_.<br><br>

      Se hai già un’idea o vuoi capire da dove partire, possiamo ragionarci insieme.
    button:
      enable: true
      label: "Scrivimi"
      link: "mailto:info@maxmamone.it"

  - title: "La mia Esperienza e Formazione"
    image: "/images/ExperienceAndGrowth.png"
    content: >-
      Dopo la laurea in Ingegneria Informatica, Elettronica e delle Telecomunicazioni, ho iniziato a lavorare come Data & Ai Engineer in Reply, collaborando con Sky Italia su progetti di data engineering e machine learning.<br>
      **Oggi sono un Freelance AI Engineer**, mi occupo di aiutare i miei clienti nell'integrazione dell'intelligenza artificiale nei loro processi e nei loro applicativi
    timeline:
      work_title: "Cronologia Lavorativa"
      education_title: "Formazione"
      work_dot_color: "amber"        # allowed: amber, green, blue, red, purple, gray
      education_dot_color: "green"   # allowed: amber, green, blue, red, purple, gray
      work:
        - company: "Elfo Srl"
          role: "Data Scientist & AI Engineer"
          period: "Dicembre 2024 - Dicembre 2025"
        - company: "ENAC Emilia Romagna"
          role: "Insegnante corso di Intelligenza Artificiale"
          period: "Maggio 2025 - Giugno 2025"
        - company: "Live Reply IT | Sky Italia"
          role: "Data & AI Engineer"
          period: "Aprile 2023 - Dicembre 2024"
        - company: "Teknomaint Srl"
          role: "Full Stack Web Developer"
          period: "Dicembre 2022 - Aprile 2023"
        - company: "ENAC Emilia Romagna"
          role: "Insegnante corso di Informatica"
          period: "Ottobre 2022 - Dicembre 2022"
      education:
        - degree: "Laurea in Ingegneria Informatica, Elettronica e delle Telecomunicazioni"
          school: "Università degli Studi di Parma"
          period: "Settembre 2019 - Dicembre 2022"
        - degree: "Certificazione Cambridge C1 Advanced"
          school: "Cambridge Assessment English"
          period: "Febbraio 2024"
        - degree: "Azure AI Engineer Associate"
          school: "Microsoft Azure"
          period: "Giugno 2025"

      note: >-
        Il mio lavoro è quello di creare codice che dialoga con modelli di machine e deep learning (anche LLM come GPT ma non solo!),
        ma mi interesso anche di integrazioni cloud (AWS, Google Cloud) e architettura di sistemi scalabili. <br><br> 
        **Mi piace restare sempre aggiornato, provare nuove tecnologie e continuare a crescere.**
        Non è solo lavoro: è uno sfida continua che mi diverte e mi motiva.

  - title: "Perché scrivo (e condivido)"
    image: "/images/DocumentAndShare.png"
    content: >-
      Nel mio tempo libero mi piace pubblicare sullo sviluppo software e intelligenza artificiale, condividendo esperimenti, intuizioni e qualche sbaglio lungo il percorso.<br><br>
      Il mio obiettivo? <br>
      **Documentare quello che imparo nella speranza che possa tornare utile anche ad altri.** Per questo, rendo open source il codice e i metodi che uso: così restano accessibili e si può discuterne e crescere insieme.<br><br>

home_contact_cta:
  title: "Fatti sentire"
  content: >-
    _**Grazie per essere passato di qui!**_<br>
    Mi fa sempre piacere entrare in contatto con imprenditori, sviluppatori, professionisti e persone curiose di tecnologia.<br><br>

    Se vuoi, puoi dare un’occhiata ai miei progetti su <u>[GitHub](https://github.com/M2Max)</u> oppure scrivermi direttamente <u>[cliccando qui](mailto:info@maxmamone.it)</u> <br><br>
    Sono disponibile per attività di consulenza, sviluppo di progetti basati su AI, automazione di processi e addestramento di modelli personalizzati sui tuoi dati, anche con soluzioni deployate in locale quando servono controllo e privacy.<br><br>

    Che tu abbia una _**domanda**_, un’idea da esplorare o un _**progetto**_ concreto da avviare, _**scrivimi**_ senza problemi: confrontarsi e imparare insieme è sempre un buon punto di partenza.
  button:
    enable: true
    label: "Scrivimi"
    link: "mailto:info@maxmamone.it"
---